...
if [ -z "$PYTHON_BIN" ]; then
  echo "Python 3.12.x is required." >&2
...
if [ -x "$ROOT/.venv/bin/python" ]; then
  if ! "$ROOT/.venv/bin/python" -c 'import sys; raise SystemExit(0 if sys.version_info[:2] == (3, 12) else 1)' >/dev/null 2>&1; then
    echo "Removing an incompatible virtual environment; Python 3.12 is mandatory."
    rm -rf "$ROOT/.venv"
  fi
fi
