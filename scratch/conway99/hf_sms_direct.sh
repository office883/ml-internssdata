...
set +e
/usr/bin/time -v timeout --signal=INT --kill-after=20s "${SOLVE_SECONDS}s" /tmp/sms/build/src/smsg -v 99 --dimacs "/tmp/conwaysms/b${b}.cnf" --initial-partition 27 72 --frequency 1 --cutoff 0 --timeout "$SOLVE_SECONDS" >"$log" 2>"$tim"
rc=$?
set -e
status=$(grep -E 'Result:|s SATISFIABLE|s UNSATISFIABLE|Instance is unknown' "$log" | tail -2 | tr '\n' ';' || true)
echo "SMS_BRANCH $b RC $rc STATUS ${status:-NONE}" | tee "/tmp/conwaysms/b${b}.summary"
tail -35 "$log" || true
tail -25 "$tim" || true
SH2
